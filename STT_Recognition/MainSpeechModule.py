...
class AudioData(object):
    ...
    def int2float(sound):
        """
        It Returns the float32 version of the input sound.
        """
...
class AudioFile(AudioSource):
    """
    Creates a new ``AudioFile`` instance given a WAV/AIFF audio file ``filename_or_fileobject``. Subclass of ``AudioSource``.
    ...
    def __init__(self, filename_or_fileobject):
        assert isinstance(filename_or_fileobject, (str, bytes)) or hasattr(filename_or_fileobject, "read"), "Given audio file must be a filename string or a file-like object"
    ...
    def __enter__(self):
        ...
        try:
            ...
            try:
                ...
                raise ValueError("Audio file could not be read as PCM WAV or AIFF/AIFF-C; check if file is corrupted or in another format")
...
class Recognizer(AudioSource):
    def __init__(self):
        ...
        self.model_base_path = 'STT_Recognition'  # Base paths for the models
        self.snowboy_model_path, self.snowboy_path = self.load_snowboy() # Load the Snowboy Hot Word Location & model
        self.vad_model, self.vad_utils = self.load_vad_model() # Load the Silero VAD model
        self.whisper_model = self.load_whisper_model(model="base")  # Load the Whisper model
        self.modelW_CPP, self.model_exe_path = self.load_whisper_modelCPP() # Load the Quantized Whisper model through whisper.cpp
    ...
    def load_snowboy(self):
        """
        Load the Snowboy hotword location and model path.
        """
        snowboy_path = os.path.join(self.model_base_path, "snowboy")
        snowboy_model_path = os.path.join(snowboy_path, "resources", "svara.pmdl")
        
        return snowboy_model_path, snowboy_path

    def human_call(self, source, timeout=None):
        """
        Listen for the wake word using Snowboy.
        """
        snowboy_location = self.snowboy_path
        snowboy_hot_word_files = [self.snowboy_model_path]
        
        # Ensure snowboy_hot_word_files is a list
        if isinstance(snowboy_hot_word_files, str):
            snowboy_hot_word_files = [snowboy_hot_word_files]
        ...
        try:
            ...
            detector.SetSensitivity(",".join(["0.42"] * len(snowboy_hot_word_files)).encode())
            ...
            console.print(f"{ERROR} Error while listening for wake word: {e}")
        ...
        try:
            ...
            while True:
                ...
                if snowboy_configuration is None:
                    ...
                    buffer, delta_time = self.human_call(snowboy_location, snowboy_hot_word_files, source, timeout)
            ...
            console.print(f"{ERROR} Error while capturing audio: {e}")

    def load_vad_model(self):
        """
        Load the Silero VAD model and return the model and utilities.
        """
        model_dir = os.path.join(self.model_base_path, 'silero')
        model_name = 'silero_vad'
        
        vad_model, vad_utils = torch.hub.load(repo_or_dir=model_dir, model=model_name, source='local')
        return vad_model, vad_utils

    def voice_activity_detection(self, audio_data):
        """
        Perform voice activity detection using the Silero VAD model.

        Parameters:
        - audio_data: The audio data to analyze for speech activity.
        
        Model Using:
        - model_name: The name of the Silero VAD model to use (default is "silero_vad").

        Returns:
        - bool: True if speech is detected, False otherwise.

        Raises:
        - RuntimeError: If there is an issue loading the model or processing the audio data.
        """
        try: 
            (get_speech_timestamps, _, _, _, _) = self.vad_utils

            wav_data = audio_data.get_wav_data(convert_rate=8000)
            newsound = np.frombuffer(wav_data, np.int16)
            audio_float32 = AudioData.int2float(newsound)
            time_stamps = get_speech_timestamps(audio_float32, self.vad_model, threshold=0.7)

            return len(time_stamps) > 0
        
        except Exception as e:
            console.print(f"{ERROR} An error occurred during voice activity detection: {e}")
            raise RuntimeError(f"Voice activity detection failed: {e}")

    def load_whisper_model(self, model="base"):
        """
        Load the Whisper model from a custom local path and return the model.
        """
        import whisper
        
        device = "cuda" if torch.cuda.is_available() else "cpu"
        
        model_path = os.path.join(self.model_base_path, 'whisper', 'models')
        whisper_model = whisper.load_model(model, device=device, download_root=model_path, in_memory=True)
        
        return whisper_model

    def recognize_whisper(self, audio_data, model="base", show_dict=True, language=None, translate=True, **transcribe_options):
        """
        ...
        try:
            if not hasattr(self, "whisper_model") or self.whisper_model is None:
                self.whisper_model = self.load_whisper_model(model)
            ...
            console.print(f"{ERROR} Error during speech-to-text conversion: {e}")

    def load_whisper_modelCPP(self):
        """
        Load the Whisper model from a custom local path and return the model.
        """
        
        modelW_CPP = os.path.join(self.model_base_path, 'whisper', 'models', "large-v3-q4_k.bin")
        model_exe_path = os.path.join(self.model_base_path, 'whisper', 'c_whisper', 'bin', 'main')
        
        return modelW_CPP, model_exe_path

    def recognize_whisperCPP(self, audio_data):
        """
        Performs speech recognition on ``audio_data`` (an ``AudioData`` instance), using Whisper.
        ...
        try:
            ...
            temp_wav = os.path.join(os.path.dirname(self.modelW_CPP), 'temp.wav')
            sf.write(temp_wav, audio_array, 16000)

            full_command = f"{self.model_exe_path} -m {self.modelW_CPP} -f {temp_wav} -np -nt -l auto"

            try:
                process = subprocess.Popen(full_command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                stdout, stderr = process.communicate()

                decoded_str = stdout.decode('utf-8').strip()
                transcription = decoded_str.replace('\n', ' ').strip()
